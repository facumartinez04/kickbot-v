...
class ViewerBot_Stability:
    def __init__(self, nb_of_threads, channel_name, proxy_file=None, proxy_imported=False, timeout=10000, type_of_proxy="http"):
        ...
        self.active_loops_lock = threading.Lock()
    ...
    def get_websocket_token(self):
        ...
        try:
            ...
            if HAS_TLS_CLIENT:
                try:
                    ...
                    if session_resp.status_code != 200:
                        logging.debug(f"Failed to establish initial session (Status {session_resp.status_code}), attempting to get token directly...")
                    else:
                    ...
                    if response.status_code == 200:
                        ...
                        if token:
                            logging.debug(f"✓ Retrieved fresh WebSocket token")
                            return token
                        else:
                            logging.error("Token endpoint returned 200 but no token in response")
                    elif response.status_code == 403:
                        logging.debug("Token endpoint returned 403 - CLIENT_TOKEN may be invalid or IP blocked")
                    else:
            ...
            try:
                ...
                if session_resp.status_code != 200:
                    logging.debug(f"Initial session request status: {session_resp.status_code} - proceeding to token retrieval anyway")
            except Exception as e:
                logging.debug(f"Initial session request failed: {e}")
            ...
            for endpoint in token_endpoints:
                try:
                    ...
                    if response.status_code == 200:
                        ...
                        if token:
                            logging.debug(f"✓ Retrieved WebSocket token from {endpoint}")
                            return token
                    elif response.status_code == 403:
                        logging.debug(f"Token endpoint {endpoint} returned 403 - CLIENT_TOKEN may be invalid")
                except Exception as e:
                    logging.debug(f"Token endpoint {endpoint} failed: {e}")
                    continue
            
            logging.debug("❌ Failed to get WebSocket token from all endpoints - check CLIENT_TOKEN validity")
            return None
            
        except Exception as e:
            logging.debug(f"Error getting WebSocket token: {e}")
            return None
    ...
    def update_status(self, state, message, proxy_count=None, proxy_loading_progress=None, startup_progress=None):
        ...
        logging.debug(f"Status updated: {self.status}")
    ...
    def stop(self):
        ...
        # Terminate all async tasks immediately
        if hasattr(self, 'active_loops_lock'):
            with self.active_loops_lock:
                for loop in self.active_loops:
                    try:
                        if loop.is_running():
                            loop.call_soon_threadsafe(
                                lambda l: [t.cancel() for t in asyncio.all_tasks(l)],
                                loop
                            )
                    except Exception as e:
                        logging.debug(f"Error cancelling tasks: {e}")
        
        # Wait a short time for threads to exit
    ...
    def send_websocket_view(self, proxy_data):
        ...
        try:
            ...
            try:
                ...
                try:
                    task = loop.create_task(self._websocket_worker(proxy_data))
    ...
    async def _websocket_worker(self, proxy_data):
        ...
        logging.debug(f"🚀 [{connection_id}] WebSocket worker started")
        
        token = None
        
        while not self.should_stop and retry_count < self.max_retry_attempts:
            try:
                # Use existing token if re-connecting, else fetch a NEW one for this specific thread
                if not token:
                    logging.debug(f"[{connection_id}] Getting new WebSocket token...")
                    token = self.get_websocket_token()
                    if not token:
                        logging.error(f"❌ [{connection_id}] Failed to get WebSocket token")
                        break
                
                logging.debug(f"[{connection_id}] Token ready, preparing connection...")
                ...
                logging.debug(f"[{connection_id}] Connecting to WebSocket via proxy...")
                ...
                async with websockets.connect(ws_url, **connect_kwargs) as websocket:
                    ...
                    logging.debug(f"📊 [{connection_id}] Active connections: {self.active_connections}")
                    ...
                    try:
                        ...
                        if not livestream_id:
                            ...
                            try:
                                ...
                                logging.debug(f"🔵 [{connection_id}] Initial response: {response}")
                                ...
                                if 'data' in response_data and 'message' in response_data['data']:
                                    ...
                                    if livestream_id:
                                        logging.debug(f"✅ [{connection_id}] Extracted livestream_id: {livestream_id}")
                            ...
                            logging.debug(f"✅ [{connection_id}] Using pre-fetched livestream_id: {livestream_id}")
                        ...
                        while not self.should_stop:
                            ...
                            if livestream_id and current_time - last_user_event_time >= 30:
                                ...
                                logging.debug(f"📺 [{connection_id}] Sent tracking event #{user_event_count}")
                        ...
                        with self.active_connections_lock:
                            self.active_connections -= 1
                        logging.debug(f"📉 [{connection_id}] Connection closed. Active connections: {self.active_connections}")
                ...
                if not self.should_stop:
                    ...
                    # No need to reset token on general connection closed
                    # It will just reconnect with the same existing token
                    # We only backoff and retry
                else:
                    break
                ...
                if e.status_code == 429:
                    ...
                    await asyncio.sleep(backoff_delay)
                    # Wait before attempting to reconnect with same token
                    # Rate limit usually affects initial token/HTTP request, but could affect WS
                    ...
                    await asyncio.sleep(backoff_delay)
                    # Force refresh token since it was 403 Forbidden
                    token = None
                else:
                    logging.error(f"WebSocket [{connection_id}] invalid status code: {e.status_code}")
                    break
            except asyncio.CancelledError:
                logging.debug(f"WebSocket [{connection_id}] task was cancelled due to bot stopping.")
                break
            except Exception as e:
                if not self.should_stop:
                    retry_count += 1
                    ...
                    await asyncio.sleep(backoff_delay)
                    # We will reuse the same token inside the try again since it was not 403
                else:
                    break
    ...
    def main(self):
        ...
        try:
            while not self.should_stop:
                ...
                # Check connection health every 5 seconds to actively replenish dead connections
                if current_time - last_connection_check >= 5:
                    ...
                    # Instead of waiting to drop below a minimum, actively replace ANY dropped connections
                    if active_count < int(self.nb_of_threads):
                        needed = int(self.nb_of_threads) - active_count
                        if needed > 0:
                            logging.debug(f"Reviving {needed} dropped connections...")
                    ...
                    # If target is lowered in real-time, kill the excess active connections
                    elif active_count > int(self.nb_of_threads):
                        excess = active_count - int(self.nb_of_threads)
                        logging.debug(f"Too many connections ({active_count}). Killing {excess} excess viewers...")
                        killed = 0
                        with self.active_loops_lock:
                            for loop in list(self.active_loops):
                                if killed >= excess:
                                    break
                                try:
                                    if loop.is_running():
                                    ...
                                    logging.debug(f"Error cancelling task to reduce viewers: {e}")
